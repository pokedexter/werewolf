...
class HamsterRule(BasicRule):
    ...
    def nextTurn_2day(self):
        ...
        # 햄스터
        hamsterPlayer = self.game.entry.getPlayersByTruecharacter(
            Truecharacter.WEREHAMSTER)[0]
    ...
    def nextTurn_Xday(self):
        ...
        if assaultVictim:
            ...
            self.assaultByWerewolfAndHamster(
                assaultVictim, victim, hamsterPlayer)
    ...
    def assaultByForecast(self, hamsterPlayer):
        logging.debug("햄빔!!")
        forecastTarget = {}
        seerPlayer = self.game.entry.getPlayersByTruecharacter(Truecharacter.SEER)[
            0]
        ...
        if seerPlayer.alive == "생존":
            ...
            if forecastTarget is not None:
                forecastTarget = self.game.entry.getCharacter(
                    forecastTarget['mystery'])
    ...
    def assaultByWerewolfAndHamster(self, assaultVictim, victim, hamsterPlayer):
        ...
        guard = None
        hunterPlayer = self.game.entry.getPlayersByTruecharacter(
            Truecharacter.BODYGUARD)[0]
